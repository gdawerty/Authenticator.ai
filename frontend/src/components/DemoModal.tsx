  ...
  const analyzeContent = async () => {
    try {
      ...
      if (contentType === 'document' && documentFile) {
        ...
        const getFileSize = () => {
          if (documentFile) return documentFile.size;
          if (imageFile) return imageFile.size;
          return 0;
        };

        const getFileType = () => {
          if (documentFile) return documentFile.type;
          if (imageFile) return imageFile.type;
          return '';
        };
        ...
        const transformedResult: AnalysisResult = {
          ...
          filename: result.filename || getFileName(),
          ...
          authenticity_assessment: {
            score: Math.round((result.authenticity_score || result.authenticity?.score || 95)),
          ...
          file_info: {
            name: result.filename || getFileName(),
            size: getFileSize(),
            type: getFileType()
          }
        };
        ...
        // Stage 2: Call ViT prediction model first
        setLoadingStage(2);
        console.log('Calling ViT prediction model...');
        
        let predictionResult = null;
        try {
          // Use ViT image classification endpoint
          ...
          if (predictionResponse.ok) {
            ...
            console.log('ViT prediction model result:', predictionResult);
          } else {
            console.warn('ViT prediction model failed, continuing with analysis...');
          }
        } catch (predictionError) {
          console.warn('ViT prediction model error:', predictionError);
        }

        // Stage 3: Extract text for additional analysis
        setLoadingStage(3);
        console.log('Extracting text from image...');
        
        const parseFormData = new FormData();
        parseFormData.append('file', imageFile);
        ...
        if (parseResponse.ok) {
          ...
          console.log('Text extracted from image:', extractedText.substring(0, 200) + '...');
        }
        ...
        // Combine ViT prediction result with analysis result
        if (predictionResult && predictionResult.classification) {
          result.prediction_model_result = predictionResult;
          console.log('Combined result with ViT prediction model:', result);
        }
              ...
              {isAnalyzing ? (
                <div className="space-y-6 w-full">
                  ...
                  <div className="text-center space-y-2">
                    ...
                    <div className="text-sm text-charcoal-400">
                      {loadingStage === 1 && (contentType === 'image' ? 'Preparing image for analysis...' : 'Preparing document for analysis...')}
                      {loadingStage === 2 && (contentType === 'image' ? 'Running ViT vision model...' : 'Extracting text with OCR...')}
                      {loadingStage === 3 && (contentType === 'image' ? 'Extracting text with OCR...' : 'Running BERT classification model...')}
                      {loadingStage === 4 && (contentType === 'image' ? 'Performing comprehensive analysis...' : 'Generating analysis results...')}
                      {loadingStage === 5 && 'Classification complete!'}
                    </div>
                  </div>
                </div>
              ) : analysisResult ? (
                <div className="w-full h-full overflow-y-auto space-y-6 py-2">
                    ...
                    {analysisResult.classification_recommended ? (
                      ...
                      <div className="relative w-full h-12 bg-charcoal-700 rounded-full overflow-hidden">
                        ...
                        <div className="absolute inset-0 flex items-center justify-center">
                          <span className="text-xl font-bold text-white">
                            {(analysisResult.authenticity_assessment?.score || 0).toFixed(1)}% Authentic
                          </span>
                        </div>
                      </div>
                    )}
                  ...
                  {analysisResult.prediction_model_result && (
                    <div className="bg-gradient-to-r from-blue-900/30 to-purple-900/30 rounded-lg p-4 border border-blue-500/30">
                      ...
                      <div className="space-y-1 text-sm text-charcoal-300">
                        <p>Model: {analysisResult.prediction_model_result.classification?.method || 
                                   (analysisResult.prediction_model_result.classification?.model_info?.model_type || 'BERT')}</p>
                        <p>Prediction: {analysisResult.prediction_model_result.classification?.prediction || 
                                        analysisResult.prediction_model_result.classification?.top_prediction?.category || 'N/A'}</p>
                        <p>Confidence: {((analysisResult.prediction_model_result.classification?.confidence || 
                                          analysisResult.prediction_model_result.classification?.top_prediction?.confidence || 0) * 100).toFixed(1)}%</p>
                        <p className="text-blue-300 text-xs mt-2">
                          ✓ Pre-analyzed with {contentType === 'image' ? 'ViT (Vision Transformer)' : 'BERT'} prediction model before main analysis
                        </p>
                      </div>
                    </div>
                  )}
